...
Therefore, we set the upper limit of the intensity to 1/3 of the peak, and the Gaussian fit is performed within that range, 
and we calculated the mean value $E$ and the standard deviation $\sigma$.
...
using CASA task {\sc immoment}. The integrated velocity range is 3.0-12.0 km s$^{-1}$, corresponding
typical velocity component of Hot core (4.8-6 km s$^{-1}$) and that of Compact ridge 
(7.2-9.6 km s$^{-1}$) \citep{Feng+2015}.
Spectroscopic data are provided by \citet{Motiyenko+2014} and 
...
\subsection{Spectrum}
スペクトルはHot core(RA$_{J2000}: 05^{\rm{h}}35^{\rm{m}}14^{\rm{s}}.580$, Dec$_{J2000}:-05^{\circ}22'31''.029$)を中心に直径1秒の領域から抽出し、ガウシアンフィットを行なった。
